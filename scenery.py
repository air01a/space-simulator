...
class Scenery:
    ...
    def read_attractor(self,name, parent=None):
        ...
        if "Atmosphere" in self.config[name].keys():
            ...
            attractor.set_atmosphere_condition(float(p0),float(h0),float(atmosphere_limit))
        ...
        if "Orbit" in self.config[name].keys() and parent!=None:
            ...
            attractor.set_orbit_parameters(parent.mu, float(a), float(e), float(i), float(raan), float(arg_pe), float(f))
