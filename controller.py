...
class Controller:
    def control_flight_path(self, orbiter):
        if (
            orbiter.name in self.attitude_control.keys()
            and len(self.attitude_control[orbiter.name]) > 0
        ):
            ...
            if (
                (param == "alt" and altitude > alt)
                or (param == "perihelion" and perigee > alt)
                or (param == "aphelion" and apogee > alt)
                or (param == "velocity" and velocity > alt)
                or (param == "time" and self.time.t >= alt)
                or (
                    param == "atperihelion"
                    and (orbiter.r.norm() - orbiter.attractor.radius) / 1000
                    < 1.05 * perigee
                )
            ):
    ...
    def get_info(self):
        ...
        if velocity > 0.005:
            ...
            info = "time thrust altitude velocity carburant orientation\n%is %.2fkN %.2fkm %.2fkm/s %ikg %ideg" % (
                int(self.time.t),
                orbiter.thrust,
                altitude,
                velocity,
